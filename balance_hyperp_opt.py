...
X_val = [ x[0] for x in Xy_balanced[:500] ]
y_val = [ x[1] for x in Xy_balanced[:500] ]
Xy_balanced = Xy_balanced[500:]

train_acc = []
val_acc = []

for balance in range(3, 6):
    nCancer = len([ i for i in Xy_balanced if i[1] == 1])
    print(len(Xy_balanced))
    while nCancer/len(Xy_balanced) < balance * 0.1 :
        nL = Xy_balanced.copy()
    ...
    X_train = X_trainL[:1000]
    y_train = y_trainL[:1000]
...
for i in range(len(train_acc)):
    plt.plot(train_acc[i], label=f'train acc (balance={3+i})')
    plt.plot(val_acc[i], label=f'val acc (balance={3+i})')
plt.xlabel('Epoch')
plt.ylabel('Accuracy')
plt.legend()
